      ...
      <div className="w-full max-w-sm z-10 space-y-10">
        ...
        <div className="space-y-3">
          ...
          >
            ...
            <span>Entrar como Coordenador</span>
          </button>
          
          <div className="grid grid-cols-1 gap-2 pt-2">
            <p className="text-[9px] font-black text-gray-500 uppercase tracking-widest text-center mb-1">Acesso Rápido Residentes</p>
            {mockTurmas.map(turma => (
              <button
                ...
                onClick={() => onLogin({ 
                  ...mockUser, 
                  role: 'student', 
                  displayName: `Residente ${turma.name.split('-')[0]}`,
                  turma_id: turma.id 
                })}
                className="w-full bg-white/5 text-white border border-white/10 rounded-xl py-3 px-6 text-xs font-bold active:scale-95 transition-transform hover:bg-white/10 flex justify-between items-center"
              >
                <span>Acessar como {turma.name.split('-')[0]}</span>
                <span className="text-brand-gold opacity-50">{turma.name.split(' ')[2]}</span>
              </button>
            ))}
          </div>
        </div>

        <p className="text-[10px] text-gray-500 text-center uppercase tracking-widest pt-4">
